...
from admin_ui.auth.session import PkceStore, build_authorize_url
...
async def start_auth(
    ...
    challenge = pkce_store.create(redirect_after_login=redirect_after_login)
...
async def token_callback(
    ...
    response = RedirectResponse(url=redirect_after_login, status_code=status.HTTP_302_FOUND)
...
async def auth_me(user: dependencies.SessionUser) -> ResponseSchema[UserInfoSchema]:
